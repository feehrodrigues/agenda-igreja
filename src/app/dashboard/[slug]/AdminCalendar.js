...
export default function AdminCalendar({ room, initialEvents, categories, allParents }) {
  ...
  // NAVEGAÇÃO DO CALENDÁRIO (CORREÇÃO DE TRAVAMENTO)
  const [view, setView] = useState('month');
  const [currentDate, setCurrentDate] = useState(new Date());

  const [searchTerm, setSearchTerm] = useState("");
  const [showResultsList, setShowResultsList] = useState(false);
  const [parentSearchTerm, setParentSearchTerm] = useState(""); // Filtro do Pai na Config
  ...
  // Filtra pais na modal de config
  ...
  const prepareForm = (evt = null, slot = null) => {
    if (evt) {
        ...
        setRruleOptions({ freq: 'NONE', interval: 1, byDay: [], monthlyType: 'day', endType: 'never', until: '', count: 1 });
    } else if (slot) {
  ...
  const handleDelete = async (mode) => {
    ...
    data.append('originalDate', selectedEvent.start.toISOString());
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      {/* RENDERIZAÇÃO DOS DEMAIS MODAIS (Mantidos iguais, apenas verifique se estão no final do return) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    ...
                    {!selectedEvent && (
                        <div className="border rounded-xl p-5 bg-slate-50 space-y-4">
                            ...
                            <div className="flex items-center gap-4 flex-wrap">
                                <div className="flex items-center gap-2"><span className="text-sm text-slate-600">A cada:</span><input type="number" min="1" value={rruleOptions.interval} onChange={e => setRruleOptions({...rruleOptions, interval: e.target.value})} className="w-16 p-2 border rounded text-center font-bold"/></div>
